...
public partial class SystemPropertyAgentHouseRoomImagesModalPage : ContentPage
{
    private const string FirebaseStorageBucket = "uttambsolutions-4ec2a.appspot.com";

    public SystemPropertyAgentHouseRoomImagesModalPage(PropertyHouseAgentDetailViewModel viewModel)
    {
        InitializeComponent();
    ...
    private async Task UploadAndSaveImageAsync(FileResult fileResult)
    {
        ...
        await ((PropertyHouseAgentDetailViewModel)BindingContext).SavePropertyHouseRoomImageasync(imagePath);
    }
}