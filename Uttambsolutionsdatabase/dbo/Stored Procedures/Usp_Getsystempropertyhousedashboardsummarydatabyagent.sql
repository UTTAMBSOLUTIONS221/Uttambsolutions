...
BEGIN
    ...
    BEGIN TRY
        ...
        (
           SELECT(
			   SELECT ISNULL((SELECT COUNT(RMS.Systempropertyhouseroomid) FROM Systempropertyhouserooms RMS INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE HSC.Propertyhouseposter =@Agentid),0) AS Propertyhouseunits,
				ISNULL((SELECT COUNT(RMS.Systempropertyhouseroomid) FROM Systempropertyhouserooms RMS INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE RMS.Isvacant =0 AND HSC.Propertyhouseposter =@Agentid),0) AS Systempropertyoccupiedroom,
				ISNULL((SELECT COUNT(RMS.Systempropertyhouseroomid) FROM Systempropertyhouserooms RMS INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE RMS.Isvacant !=0 AND HSC.Propertyhouseposter =@Agentid),0) AS Systempropertyvacantroom,
				ISNULL((SELECT SUM(RMS.Systempropertyhousesizerent) FROM Systempropertyhouserooms RMS INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE HSC.Propertyhouseposter =@Agentid),0) AS Expectedcollections,
				ISNULL((SELECT SUM(INV.Balance) FROM Monthlyrentinvoices INV INNER JOIN Systempropertyhouserooms RMS ON RMS.Systempropertyhouseroomid=INV.Propertyhouseroomid INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE INV.Invoicestatus='Subsequentinvoice' AND INV.Paidstatus='PARTIALLY PAID' AND HSC.Propertyhouseposter =@Agentid),0) AS Rentarrears,
				ISNULL((SELECT SUM(INV.Paidamount) FROM Monthlyrentinvoices INV INNER JOIN Systempropertyhouserooms RMS ON RMS.Systempropertyhouseroomid=INV.Propertyhouseroomid INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE INV.Invoicestatus='Subsequentinvoice' AND HSC.Propertyhouseposter =@Agentid),0) AS Collectedpayments,
				ISNULL((SELECT SUM(INV.Balance) FROM Monthlyrentinvoices INV INNER JOIN Systempropertyhouserooms RMS ON RMS.Systempropertyhouseroomid=INV.Propertyhouseroomid INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE INV.Invoicestatus='Subsequentinvoice' AND INV.Paidstatus='NOT PAID' AND HSC.Propertyhouseposter =@Agentid),0) AS Uncollectedpayments,
				ISNULL((SELECT SUM(MTRS.Movedmeter) FROM Systempropertyhouseroommeters MTRS INNER JOIN Systempropertyhouserooms RMS ON RMS.Systempropertyhouseroomid=MTRS.Systempropertyhouseroomid INNER JOIN Systempropertyhouses HSC ON RMS.Systempropertyhouseid=HSC.Propertyhouseid WHERE HSC.Propertyhouseposter =@Agentid),0) AS Consumedmeters,
				...
				WHERE HSC.Propertyhouseposter = @Agentid
