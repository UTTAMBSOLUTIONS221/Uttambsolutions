   ...
   BEGIN
	DECLARE 
		...
		SET @OwnerTenantAgreementDetailData = (SELECT (SELECT HSC.Propertyhouseid,HSC.Propertyhouseowner,HSC.Propertyhousename AS Propertyhousename,OWN.Firstname+' '+OWN.Lastname AS Fullname,OWN.Phonenumber,OWN.Emailaddress,Systemcounty.Countyname,Systemsubcounty.Subcountyname,Systemsubcountyward.Subcountywardname,
			ISNULL(SOA.Datecreated,GETDATE()) AS OwnerDatecreated,ISNULL(SOT.Datecreated,GETDATE()) AS TenantDatecreated,ISNULL(SOA.Signatureimageurl,'') AS OwnerSignatureimageurl,ISNULL(SOT.Signatureimageurl,'') AS TenantSignatureimageurl
			...
			LEFT JOIN Systempropertyhouseagreements SOT ON OWN.Userid=SOT.Ownerortenantid
